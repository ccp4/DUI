    ...
    def paintEvent(self, event):
        if( self.img == None ):
            ...
            if( self.flat_data_lst != None and self.my_parent.chk_box_show.checkState()):
                ...
                for pos in xrange(self.flat_data_lst.n_idx):
                    x = float(self.flat_data_lst.arr[0, pos])
                    y = float(self.flat_data_lst.arr[1, pos])
                    width = float(self.flat_data_lst.arr[2, pos])
                    height = float(self.flat_data_lst.arr[3, pos])
                    rectangle = QRectF(x * self.my_scale, y * self.my_scale,
                                       width * self.my_scale, height * self.my_scale)
                    painter.drawRect(rectangle)

                    if( self.flat_data_lst.hkl != None and self.my_scale > self.my_parent.t_hold ):
                        local_hkl = "(" + str(self.flat_data_lst.hkl[0, pos]) + ", " + \
                                          str(self.flat_data_lst.hkl[1, pos]) + ", " + \
                                          str(self.flat_data_lst.hkl[2, pos]) + ")"
                        painter.drawText( QPoint(int((x + width) * self.my_scale),
                                          int(y * self.my_scale)),  local_hkl)
...
class MyImgWin(QWidget):
    ...
    def ini_reflection_table(self, pckl_file_path):
        if( pckl_file_path != None and self.img_select.maximum() > 0 ):

            del self.flat_data_lst
            ...
            bbox_arr_col= np.array(bbox_col)
            self.flat_data_lst = []

            try:
                hkl_col = map(list, table["miller_index"])
                hkl_arr_col = np.array(hkl_col)
                arr_2d_col = np.zeros((n_refs, 9), dtype = np.int)
                arr_2d_col[:, 0:6] = bbox_arr_col[:, :]
                arr_2d_col[:, 6:9] = hkl_arr_col[:, :]
                ...
                for img_num in xrange(self.img_select.maximum()):
                    tmp_data = flat_data()
                    tmp_arr = np.zeros((4, n_refs), dtype = np.int)
                    tmp_data.arr = tmp_arr
                    tmp_hkl = np.zeros((3, n_refs), dtype = np.int)
                    tmp_data.hkl = tmp_hkl
                    tmp_data.n_idx = 0
                    self.flat_data_lst.append(tmp_data)

                for arr_row in arr_2d_col:
                    local_bbox = arr_row[0:6]
                    ...
                    local_hkl = arr_row[6:9]

                    for idx in xrange( z_boud[0], z_boud[1] ):

                        [x_ini, y_ini, width, height]

                        self.flat_data_lst[idx].n_idx += 1
                        arr_pos = self.flat_data_lst[idx].n_idx
                        self.flat_data_lst[idx].arr[0, arr_pos] = x_ini
                        self.flat_data_lst[idx].arr[1, arr_pos] = y_ini
                        self.flat_data_lst[idx].arr[2, arr_pos] = width
                        self.flat_data_lst[idx].arr[3, arr_pos] = height
                        self.flat_data_lst[idx].hkl[:, arr_pos] = local_hkl[:]

                print "\n building flat_data_lst (diff time) =", time_now() - firts_time, "\n"


            except:
                arr_2d_col = np.zeros((n_refs, 6), dtype = np.int)
                arr_2d_col[:, 0:6] = bbox_arr_col[:, :]
